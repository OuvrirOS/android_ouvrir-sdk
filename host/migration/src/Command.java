    ...
    protected static boolean filter(String uri, Setting setting) {
        switch (uri) {
            case SettingsConstants.SYSTEM:
                if (!OuvrirSettings.System.isLegacySetting(setting.getKey())) {
                    return true;
                }
                break;
            case SettingsConstants.SECURE:
                if (!OuvrirSettings.Secure.isLegacySetting(setting.getKey())) {
                    return true;
                }
                break;
            case SettingsConstants.GLOBAL:
                if (!OuvrirSettings.Global.isLegacySetting(setting.getKey())) {
                    return true;
                }
                break;
        }
