...
public class PictureAdjustmentController extends LiveDisplayFeature {
    ...
    public PictureAdjustmentController(Context context, Handler handler) {
        ...
        mHardware = OuvrirHardwareManager.getInstance(context);
        mHasDisplayModes = mHardware.isSupported(OuvrirHardwareManager.FEATURE_DISPLAY_MODES);

        boolean usePA = mHardware.isSupported(OuvrirHardwareManager.FEATURE_PICTURE_ADJUSTMENT);
        if (usePA) {
    ...
    public void onStart() {
        ...
        registerSettings(
                OuvrirSettings.System.getUriFor(OuvrirSettings.System.DISPLAY_PICTURE_ADJUSTMENT));
    }
    ...
    private SparseArray<HSIC> unpackPreference() {
        ...
        String pref = getString(OuvrirSettings.System.DISPLAY_PICTURE_ADJUSTMENT);
        if (pref != null) {
    ...
    private void packPreference(final SparseArray<HSIC> modes) {
        ...
        putString(OuvrirSettings.System.DISPLAY_PICTURE_ADJUSTMENT, sb.toString());
    }
