...
import org.ouvriros.platform.internal.OuvrirSystemService;
import org.ouvriros.platform.internal.common.UserContentObserver;
import org.ouvriros.platform.internal.display.TwilightTracker.TwilightListener;
import org.ouvriros.platform.internal.display.TwilightTracker.TwilightState;
...
import static ouvriros.hardware.LiveDisplayManager.MODE_FIRST;
import static ouvriros.hardware.LiveDisplayManager.MODE_LAST;
import static ouvriros.hardware.LiveDisplayManager.MODE_OFF;
 ...
 * and calibration. It interacts with OuvrirHardwareService to relay
 * changes down to the lower layers.
 */
public class LiveDisplayService extends OuvrirSystemService {
    ...
    public String getFeatureDeclaration() {
        return OuvrirContextConstants.Features.LIVEDISPLAY;
    }
    ...
    @Override
    public void onStart() {
        publishBinderService(OuvrirContextConstants.OUVRIR_LIVEDISPLAY_SERVICE, mBinder);
    }
    ...
    public void onBootPhase(int phase) {
        if (phase == PHASE_BOOT_COMPLETED) {
            ...
            int defaultMode = mContext.getResources().getInteger(
                    org.ouvriros.platform.internal.R.integer.config_defaultLiveDisplayMode);
            ...
            Intent intent = new Intent(ouvriros.content.Intent.ACTION_INITIALIZE_LIVEDISPLAY);
    ...
    private final IBinder mBinder = new ILiveDisplayService.Stub() {
        ...
        public boolean setMode(int mode) {
            mContext.enforceCallingOrSelfPermission(
                    ouvriros.platform.Manifest.permission.MANAGE_LIVEDISPLAY, null);
            if (!mConfig.hasModeSupport()) {
                return false;
            }
    ...
    private final class ModeObserver extends UserContentObserver {
        ...
        private final Uri MODE_SETTING =
                OuvrirSettings.System.getUriFor(OuvrirSettings.System.DISPLAY_TEMPERATURE_MODE);
        ...
        int getMode() {
            return getInt(OuvrirSettings.System.DISPLAY_TEMPERATURE_MODE,
                    mConfig.getDefaultMode());
        }

        boolean setMode(int mode) {
            if (mConfig.hasFeature(mode) && mode >= MODE_FIRST && mode <= MODE_LAST) {
                putInt(OuvrirSettings.System.DISPLAY_TEMPERATURE_MODE, mode);
    ...
    private int getSunsetCounter() {
        ...
        return OuvrirSettings.System.getIntForUser(mContext.getContentResolver(),
    ...
    private void updateSunsetCounter(int count) {
        OuvrirSettings.System.putIntForUser(mContext.getContentResolver(),
                OuvrirSettings.System.LIVE_DISPLAY_HINTED,
                count,
                UserHandle.USER_CURRENT);
    ...
    private void nudge() {
        ...
        if (counter == 0) {
            ...
            final Intent intent = new Intent(OuvrirSettings.ACTION_LIVEDISPLAY_SETTINGS);
            PendingIntent result = PendingIntent.getActivity(mContext, 0, intent,
                    PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_UPDATE_CURRENT);
            Notification.Builder builder = new Notification.Builder(mContext)
                    .setContentTitle(mContext.getResources().getString(
                            org.ouvriros.platform.internal.R.string.live_display_title))
                    .setContentText(mContext.getResources().getString(
                            org.ouvriros.platform.internal.R.string.live_display_hint))
                    .setSmallIcon(org.ouvriros.platform.internal.R.drawable.ic_livedisplay_notif)
