...
import org.ouvriros.platform.internal.common.OuvrirSystemServiceHelper;

/**
 * Base Ouvrir System Server which handles the starting and states of various Ouvrir
 ...
 */
public class OuvrirSystemServer {
    private static final String TAG = "OuvrirSystemServer";
    private Context mSystemContext;
    private OuvrirSystemServiceHelper mSystemServiceHelper;

    private static final String ENCRYPTING_STATE = "trigger_restart_min_framework";
    private static final String ENCRYPTED_STATE = "1";

    public OuvrirSystemServer(Context systemContext) {
        mSystemContext = systemContext;
        mSystemServiceHelper = new OuvrirSystemServiceHelper(mSystemContext);
    }
    ...
    private void run() {
        ...
        try {
            ...
            Slog.e("System", "******************************************");
            Slog.e("System", "************ Failure starting ouvrir system services", ex);
            throw ex;
        }
    }
    ...
    private void startServices() {
        ...
        String[] externalServices = context.getResources().getStringArray(
                org.ouvriros.platform.internal.R.array.config_externalOuvrirServices);

        for (String service : externalServices) {
            try {
                Slog.i(TAG, "Attempting to start service " + service);
                OuvrirSystemService ouvrirSystemService =  mSystemServiceHelper.getServiceFor(service);
                if (context.getPackageManager().hasSystemFeature(
                        ouvrirSystemService.getFeatureDeclaration())) {
                    if (coreAppsOnly() && !ouvrirSystemService.isCoreService()) {
                        Slog.d(TAG, "Not starting " + service +
                                " - only parsing core apps");
                    } else {
                        Slog.i(TAG, "Starting service " + service);
                        ssm.startService(ouvrirSystemService.getClass());
                    }
                } else {
