...
import static ouvriros.hardware.LiveDisplayManager.MODE_OUTDOOR;
...
public class OutdoorModeController extends LiveDisplayFeature {
    ...
    public OutdoorModeController(Context context, Handler handler) {
        ...
        mUseOutdoorMode = mHardware.isSupported(OuvrirHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT);

        mDefaultOutdoorLux = mContext.getResources().getInteger(
                org.ouvriros.platform.internal.R.integer.config_outdoorAmbientLux);
        mOutdoorLuxHysteresis = mContext.getResources().getInteger(
                org.ouvriros.platform.internal.R.integer.config_outdoorAmbientLuxHysteresis);
        mDefaultAutoOutdoorMode = mContext.getResources().getBoolean(
                org.ouvriros.platform.internal.R.bool.config_defaultAutoOutdoorMode);
    }
    ...
    public void onStart() {
        ...
        registerSettings(
                OuvrirSettings.System.getUriFor(OuvrirSettings.System.DISPLAY_AUTO_OUTDOOR_MODE));
    }
    ...
    protected synchronized void onScreenStateChanged() {
        ...
        if (!isScreenOn() && getMode() != MODE_OUTDOOR) {
            mIsOutdoor = false;
            mHardware.set(OuvrirHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT, false);
        }
    }
    ...
    public void dump(PrintWriter pw) {
        ...
        pw.println("    hardware state=" +
                mHardware.get(OuvrirHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT));
    ...
    private synchronized void updateOutdoorMode() {
        ...
        if (isScreenOn()) {
            ...
            mHardware.set(OuvrirHardwareManager.FEATURE_SUNLIGHT_ENHANCEMENT, enabled);
        }
    }
    ...
    boolean setAutomaticOutdoorModeEnabled(boolean enabled) {
        ...
        putBoolean(OuvrirSettings.System.DISPLAY_AUTO_OUTDOOR_MODE, enabled);
        return true;
    }

    boolean isAutomaticOutdoorModeEnabled() {
        return mUseOutdoorMode && (mNightDisplayAvailable ||
                getBoolean(OuvrirSettings.System.DISPLAY_AUTO_OUTDOOR_MODE,
