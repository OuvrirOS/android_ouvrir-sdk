...
import java.util.Map;
 ...
 public class OuvrirSettingsProviderTest extends AndroidTestCase {
     private static final String TAG = "OuvrirSettingsProviderTest";
     ...
     @MediumTest
     public void testMigrateOuvrirSettingsForOtherUser() {
     ...
     private void testMigrateSettingsForUser(int userId) {
         ...
         Settings.System.putStringForUser(mContentResolver,
                 OuvrirSettingsProvider.LegacyOuvrirSettings.STATUS_BAR_QUICK_QS_PULLDOWN,
                 expectedPullDownValue, userId);

         final int expectedKeyboardBrightness = 4;
         Settings.Secure.putIntForUser(mContentResolver,
                 OuvrirSettingsProvider.LegacyOuvrirSettings.KEYBOARD_BRIGHTNESS,
                 expectedKeyboardBrightness, userId);

         Bundle arg = new Bundle();
         arg.putInt(OuvrirSettings.CALL_METHOD_USER_KEY, userId);
         IContentProvider contentProvider = mContentResolver.acquireProvider(
                 OuvrirSettings.AUTHORITY);

         try{
             // Trigger migrate settings for guest
             contentProvider.call(mContentResolver.getPackageName(),
                     OuvrirSettings.CALL_METHOD_MIGRATE_SETTINGS_FOR_USER, null, arg);
         } catch (RemoteException ex) {
             fail("Failed to trigger settings migration due to RemoteException");
         }

         // Check values
         final String actualPullDownValue = OuvrirSettings.System.getStringForUser(mContentResolver,
                 OuvrirSettings.System.QS_QUICK_PULLDOWN, userId);
         assertEquals(expectedPullDownValue, actualPullDownValue);

         final int actualKeyboardBrightness = OuvrirSettings.Secure.getIntForUser(mContentResolver,
                 OuvrirSettings.Secure.KEYBOARD_BRIGHTNESS, -1, userId);
     ...
     public void testBulkInsertSuccess() {
         ...
         testBulkInsertForUri(OuvrirSettings.System.CONTENT_URI, contentValues, keyValues);
         testBulkInsertForUri(OuvrirSettings.Secure.CONTENT_URI, contentValues, keyValues);
         testBulkInsertForUri(OuvrirSettings.Global.CONTENT_URI, contentValues, keyValues);
     }
     ...
     public void testInsertUpdateDeleteSuccess() {
         //testInsertUpdateDeleteForUri(OuvrirSettings.System.CONTENT_URI);
         testInsertUpdateDeleteForUri(OuvrirSettings.Secure.CONTENT_URI);
         testInsertUpdateDeleteForUri(OuvrirSettings.Global.CONTENT_URI);
     }
