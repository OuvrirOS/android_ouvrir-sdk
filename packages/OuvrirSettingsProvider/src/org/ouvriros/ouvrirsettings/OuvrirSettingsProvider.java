...
package org.ouvriros.ouvrirsettings;
...
/**
 * The OuvrirSettingsProvider serves as a {@link ContentProvider} for Ouvrir specific settings
 */
public class OuvrirSettingsProvider extends ContentProvider {
    public static final String TAG = "OuvrirSettingsProvider";
    ...
    public static final String PREF_HAS_MIGRATED_OUVRIR_SETTINGS =
            "migrated_settings_to_ouvrir_17_0";

    private static final Bundle NULL_SETTING = Bundle.forPair("value", null);

    // Each defined user has their own settings
    protected final SparseArray<OuvrirDatabaseHelper> mDbHelpers = new SparseArray<OuvrirDatabaseHelper>();
    ...
    static {
        sUriMatcher.addURI(OuvrirSettings.AUTHORITY, OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SYSTEM,
                SYSTEM);
        sUriMatcher.addURI(OuvrirSettings.AUTHORITY, OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SECURE,
                SECURE);
        sUriMatcher.addURI(OuvrirSettings.AUTHORITY, OuvrirDatabaseHelper.OuvrirTableNames.TABLE_GLOBAL,
                GLOBAL);
        sUriMatcher.addURI(OuvrirSettings.AUTHORITY, OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SYSTEM +
    ...
    public boolean onCreate() {
        if (LOCAL_LOGV) Log.d(TAG, "Creating OuvrirSettingsProvider");
        ...
        mUriBuilder.authority(OuvrirSettings.AUTHORITY);
    ...
    /**
     * Migrates Ouvrir settings for all existing users if this has not been run before.
     */
    private void migrateOuvrirSettingsForExistingUsersIfNeeded() {
        boolean hasMigratedOuvrirSettings = mSharedPrefs.getBoolean(PREF_HAS_MIGRATED_OUVRIR_SETTINGS,
                false);

        if (!hasMigratedOuvrirSettings) {
            ...
            for (UserInfo user : mUserManager.getUsers()) {
                migrateOuvrirSettingsForUser(user.id);
            }

            mSharedPrefs.edit().putBoolean(PREF_HAS_MIGRATED_OUVRIR_SETTINGS, true).commit();
    ...
    /**
     * Migrates Ouvrir settings for a specific user.
     * @param userId The id of the user to run Ouvrir settings migration for.
     */
    private void migrateOuvrirSettingsForUser(int userId) {
        synchronized (this) {
            if (LOCAL_LOGV) Log.d(TAG, "Ouvrir settings will be migrated for user id: " + userId);

            // Rename database files (if needed)
            OuvrirDatabaseHelper dbHelper = mDbHelpers.get(userId);
            if (dbHelper != null) {
                dbHelper.close();
                mDbHelpers.delete(userId);
            }
            OuvrirDatabaseHelper.migrateDbFiles(getContext(), userId);
            if (dbHelper != null) {
            ...
            int rowsMigrated = migrateOuvrirSettingsForTable(userId,
                    OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SYSTEM, OuvrirSettings.System.LEGACY_SYSTEM_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Ouvrir system table");

            // Migrate secure settings
            rowsMigrated = migrateOuvrirSettingsForTable(userId,
                    OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SECURE, OuvrirSettings.Secure.LEGACY_SECURE_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Ouvrir secure table");

            // Migrate global settings
            rowsMigrated = migrateOuvrirSettingsForTable(userId,
                    OuvrirDatabaseHelper.OuvrirTableNames.TABLE_GLOBAL, OuvrirSettings.Global.LEGACY_GLOBAL_SETTINGS);
            if (LOCAL_LOGV) Log.d(TAG, "Migrated " + rowsMigrated + " to Ouvrir global table");
        }
    }

    /**
     * Migrates Ouvrir settings for a specific table and user id.
     * @param userId The id of the user to run Ouvrir settings migration for.
     * @param tableName The name of the table to run Ouvrir settings migration on.
     * @param settings An array of keys to migrate from {@link Settings} to {@link OuvrirSettings}
     * @return Number of rows migrated.
     */
    private int migrateOuvrirSettingsForTable(int userId, String tableName, String[] settings) {
        ...
        for (String settingsKey : settings) {
            ...
            if (tableName.equals(OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SYSTEM)) {
                settingsValue = Settings.System.getStringForUser(contentResolver, settingsKey,
                        userId);
            }
            else if (tableName.equals(OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SECURE)) {
                settingsValue = Settings.Secure.getStringForUser(contentResolver, settingsKey,
                        userId);
                if (settingsValue != null && settingsKey.equals(OuvrirSettings.Secure.STATS_COLLECTION)
                        && OuvrirSettings.Secure.getStringForUser(contentResolver, settingsKey, userId)
                        != null) {
            ...
            else if (tableName.equals(OuvrirDatabaseHelper.OuvrirTableNames.TABLE_GLOBAL)) {
    ...
    public Bundle call(String method, String request, Bundle args) {
        ...
        if (args != null) {
            int reqUser = args.getInt(OuvrirSettings.CALL_METHOD_USER_KEY, callingUserId);
        ...
        switch (method) {
            // Migrate methods
           case OuvrirSettings.CALL_METHOD_MIGRATE_SETTINGS:
                migrateOuvrirSettingsForExistingUsersIfNeeded();
                return null;
           case OuvrirSettings.CALL_METHOD_MIGRATE_SETTINGS_FOR_USER:
                migrateOuvrirSettingsForUser(callingUserId);
                return null;

            // Get methods
            case OuvrirSettings.CALL_METHOD_GET_SYSTEM:
                return lookupSingleValue(callingUserId, OuvrirSettings.System.CONTENT_URI,
                        request);
            case OuvrirSettings.CALL_METHOD_GET_SECURE:
                return lookupSingleValue(callingUserId, OuvrirSettings.Secure.CONTENT_URI,
                        request);
            case OuvrirSettings.CALL_METHOD_GET_GLOBAL:
                return lookupSingleValue(callingUserId, OuvrirSettings.Global.CONTENT_URI,
                        request);

            // Put methods
            case OuvrirSettings.CALL_METHOD_PUT_SYSTEM:
                enforceWritePermission(ouvriros.platform.Manifest.permission.WRITE_SETTINGS);
                callHelperPut(callingUserId, OuvrirSettings.System.CONTENT_URI, request, args);
                return null;
            case OuvrirSettings.CALL_METHOD_PUT_SECURE:
                enforceWritePermission(
                        ouvriros.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, OuvrirSettings.Secure.CONTENT_URI, request, args);
                return null;
            case OuvrirSettings.CALL_METHOD_PUT_GLOBAL:
                enforceWritePermission(
                        ouvriros.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                callHelperPut(callingUserId, OuvrirSettings.Global.CONTENT_URI, request, args);
                return null;

            // List methods
            case OuvrirSettings.CALL_METHOD_LIST_SYSTEM:
                return callHelperList(callingUserId, OuvrirSettings.System.CONTENT_URI);
            case OuvrirSettings.CALL_METHOD_LIST_SECURE:
                return callHelperList(callingUserId, OuvrirSettings.Secure.CONTENT_URI);
            case OuvrirSettings.CALL_METHOD_LIST_GLOBAL:
                return callHelperList(callingUserId, OuvrirSettings.Global.CONTENT_URI);

            // Delete methods
            case OuvrirSettings.CALL_METHOD_DELETE_SYSTEM:
                enforceWritePermission(ouvriros.platform.Manifest.permission.WRITE_SETTINGS);
                return callHelperDelete(callingUserId, OuvrirSettings.System.CONTENT_URI,
                        request);
            case OuvrirSettings.CALL_METHOD_DELETE_SECURE:
                enforceWritePermission(
                        ouvriros.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, OuvrirSettings.Secure.CONTENT_URI,
                        request);
            case OuvrirSettings.CALL_METHOD_DELETE_GLOBAL:
                enforceWritePermission(
                        ouvriros.platform.Manifest.permission.WRITE_SECURE_SETTINGS);
                return callHelperDelete(callingUserId, OuvrirSettings.Global.CONTENT_URI,
                        request);
        }
    ...
    private Cursor queryForUser(int userId, Uri uri, String[] projection, String selection,
        ...
        OuvrirDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName, userId));
    ...
    private Uri insertForUser(int userId, Uri uri, ContentValues values) {
        ...
        if (OuvrirDatabaseHelper.OuvrirTableNames.TABLE_GLOBAL.equals(tableName)) {
            validateGlobalSettingNameValue(name, value);
        } else if (OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SYSTEM.equals(tableName)) {
            validateSystemSettingNameValue(name, value);
        } else if (OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SECURE.equals(tableName)) {
    ...
    private int deleteForUser(int callingUserId, Uri uri, String selection,
        ...
        if (!TextUtils.isEmpty(selection) && selectionArgs.length > 0) {
            ...
            OuvrirDatabaseHelper dbHelper = getOrEstablishDatabase(getUserIdForTable(tableName,
                    callingUserId));
    ...
    public int update(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        // NOTE: update() is never called by the front-end OuvrirSettings API, and updates that
    ...
    /**
     * Tries to get a {@link OuvrirDatabaseHelper} for the specified user and if it does not exist, a
     * new instance of {@link OuvrirDatabaseHelper} is created for the specified user and returned.
     * @param callingUser
     * @return
     */
    private OuvrirDatabaseHelper getOrEstablishDatabase(int callingUser) {
    ...
    /**
     * Check if a {@link OuvrirDatabaseHelper} exists for a user and if it doesn't, a new helper is
    ...
    private void establishDbTracking(int userId) {
        ...
        synchronized (this) {
            ...
            if (LOCAL_LOGV) {
                Log.i(TAG, "Checking ouvrir settings db helper for user " + userId);
            }
            if (dbHelper == null) {
                if (LOCAL_LOGV) {
                    Log.i(TAG, "Installing new ouvrir settings db helper for user " + userId);
                }
                dbHelper = new OuvrirDatabaseHelper(getContext(), userId);
    ...
    private void checkWritePermissions(String tableName) {
        ...
        final boolean protectedTable =
                OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SECURE.equals(tableName) ||
                OuvrirDatabaseHelper.OuvrirTableNames.TABLE_GLOBAL.equals(tableName);
        ...
        throw new SecurityException(
                String.format("Permission denial: writing to ouvrir settings requires %1$s",
                        ouvriros.platform.Manifest.permission.WRITE_SECURE_SETTINGS));
    }
    ...
    private String getTableNameFromUriMatchCode(int code) {
        switch (code) {
            case SYSTEM:
            case SYSTEM_ITEM_NAME:
                return OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SYSTEM;
            case SECURE:
            case SECURE_ITEM_NAME:
                return OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SECURE;
            case GLOBAL:
            case GLOBAL_ITEM_NAME:
                return OuvrirDatabaseHelper.OuvrirTableNames.TABLE_GLOBAL;
            default:
                throw new IllegalArgumentException("Invalid uri match code: " + code);
        }
    }
    ...
    private int getUserIdForTable(String tableName, int userId) {
        return OuvrirDatabaseHelper.OuvrirTableNames.TABLE_GLOBAL.equals(tableName) ?
                UserHandle.USER_SYSTEM : userId;
    }

    /**
     * Modify setting version for an updated table before notifying of change. The
     * {@link OuvrirSettings} class uses these to provide client-side caches.
     * @param uri to send notifications for
     * @param userId
     */
    private void notifyChange(Uri uri, String tableName, int userId) {
        String property = null;
        final boolean isGlobal = tableName.equals(OuvrirDatabaseHelper.OuvrirTableNames.TABLE_GLOBAL);
        if (tableName.equals(OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SYSTEM)) {
            property = OuvrirSettings.System.SYS_PROP_OUVRIR_SETTING_VERSION;
        } else if (tableName.equals(OuvrirDatabaseHelper.OuvrirTableNames.TABLE_SECURE)) {
            property = OuvrirSettings.Secure.SYS_PROP_OUVRIR_SETTING_VERSION;
        } else if (isGlobal) {
            property = OuvrirSettings.Global.SYS_PROP_OUVRIR_SETTING_VERSION;
        }
    ...
    private void validateGlobalSettingNameValue(String name, String value) {
        OuvrirSettings.Validator validator = OuvrirSettings.Global.VALIDATORS.get(name);
    ...
    private void validateSystemSettingNameValue(String name, String value) {
        OuvrirSettings.Validator validator = OuvrirSettings.System.VALIDATORS.get(name);
        if (validator == null) {
    ...
    private void validateSecureSettingValue(String name, String value) {
        OuvrirSettings.Validator validator = OuvrirSettings.Secure.VALIDATORS.get(name);
