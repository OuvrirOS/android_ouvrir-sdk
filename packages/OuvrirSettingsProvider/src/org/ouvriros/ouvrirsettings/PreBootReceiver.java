...
public class PreBootReceiver extends BroadcastReceiver{
    private static final String TAG = "OuvrirSettingsReceiver";
    private static final boolean LOCAL_LOGV = false;

    @Override
    public void onReceive(Context context, Intent intent) {
        if (LOCAL_LOGV) {
            Log.d(TAG, "Received pre boot intent. Attempting to migrate Ouvrir settings.");
        }
        ...
        try {
            contentProvider.call(contentResolver.getAttributionSource(),
                    OuvrirSettings.AUTHORITY,
                    OuvrirSettings.CALL_METHOD_MIGRATE_SETTINGS, null, null);
