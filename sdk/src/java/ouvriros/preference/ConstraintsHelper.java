...
public class ConstraintsHelper {
    ...
    public ConstraintsHelper(Context context, AttributeSet attrs, Preference pref) {
        ...
        TypedArray a = context.getResources().obtainAttributes(attrs,
                R.styleable.ouvrir_SelfRemovingPreference);
        mSummaryMinLines = a.getInteger(R.styleable.ouvrir_SelfRemovingPreference_minSummaryLines, -1);
        String replacesKey = a.getString(R.styleable.ouvrir_SelfRemovingPreference_replacesKey);
        if (replacesKey != null) {
    ...
    private boolean checkConstraints() {
        ...
        try {
            ...
            boolean rOwner = a.getBoolean(R.styleable.ouvrir_SelfRemovingPreference_requiresOwner, false);
            if (rOwner && UserHandle.myUserId() != UserHandle.USER_SYSTEM) {
                return false;
            }

            // Check if a specific package is installed
            String rPackage = a.getString(R.styleable.ouvrir_SelfRemovingPreference_requiresPackage);
            if (rPackage != null) {
            ...
            String rAction = a.getString(R.styleable.ouvrir_SelfRemovingPreference_requiresAction);
            if (rAction != null) {
            ...
            String rFeature = a.getString(R.styleable.ouvrir_SelfRemovingPreference_requiresFeature);
            if (rFeature != null) {
                boolean negated = isNegated(rFeature);
                if (negated) {
                    rFeature = rFeature.substring(1);
                }
                boolean available = rFeature.startsWith("ouvrirhardware:") ?
                        OuvrirHardwareManager.getInstance(mContext).isSupported(
                                rFeature.substring("ouvrirhardware:".length())) :
            ...
            String rProperty = a.getString(R.styleable.ouvrir_SelfRemovingPreference_requiresProperty);
            if (rProperty != null) {
            ...
            TypedValue tv = a.peekValue(R.styleable.ouvrir_SelfRemovingPreference_requiresConfig);
            if (tv != null && tv.resourceId != 0) {
                if (tv.type == TypedValue.TYPE_STRING &&
                    ...
                    int mask = a.getInt(
                            R.styleable.ouvrir_SelfRemovingPreference_requiresConfigMask, -1);
